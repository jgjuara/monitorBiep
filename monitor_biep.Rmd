...
BIEP_EDUCA <- fread("curl https://www.argentina.gob.ar/sites/default/files/biep_puestos_de_trabajo_-_apn_s_nivel_educ_2009-202010.zip | funzip",
                    dec = ",")
...
sinep2 <- BIEP_EDUCA %>% 
    filter(Escalafón == "303 - SINEP") %>% 
  group_by(Jurisdiccion, Organismo, Género) %>% 
  summarise(
...
sinep4 <- BIEP_EDUCA %>% 
    filter(Escalafón == "303 - SINEP") %>% 
  group_by(Jurisdiccion, Organismo, `Tipo Contratación`) %>% 
  summarise( n = sum(Puestos),
            "Salario Bruto Promedio (pesos corrientes)" = round(weighted.mean(SBP, Puestos), 0),
...
BIEP_EDUCA_temp <- BIEP_EDUCA_temp %>% 
  mutate(`Período Fecha` = lubridate::ymd(paste0(Período, "01")))
...
base_BIEP_EDUCA <- BIEP_EDUCA_temp %>% 
  ...
  group_by(`Período Fecha`, Escalafón) %>% 
  summarise(base2017 = weighted.mean(CFP, Puestos)) %>% 
  filter(`Período Fecha` == "2017-11-01") %>% 
  left_join(ipc) %>% 
  mutate(
...
cfp_timeGraf <- serie_variacion_CFP %>% 
  ggplot(aes(x = `Período Fecha.x`, y = nivelSalarioReal, color = Escalafón, label = Escalafón))+
  geom_line()+
  geom_point()+
  scale_color_viridis_d()+
  theme_minimal()+
  scale_x_date(breaks = unique(serie_variacion_CFP$`Período Fecha.x`))+
  xlab("Año")+
